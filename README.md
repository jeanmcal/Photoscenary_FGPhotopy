...
1. Install the required toolchain (Python, dependencies, and external tools).
...
## Installation Steps
  ...
  Run the script with the `--version` flag to check if ImageMagick and NVTT are correctly installed:
  ```
  python photoscenary.py --version
  ```

## Usage
  Photoscenary.py is a command-line tool. Use the following commands to run it:
  ```
